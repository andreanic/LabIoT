...
import it.lab.iot.sensor.constants.SensorConstants;
...
@Service
public class SensorService extends SensorConstants implements ISensorService  {
	...
	public List<SensorValueDTO> getLast20ValuesBySensorId(String sensorId) throws BaseException {
		...
		List<SensorValues> sensors = sensorValuesRepository.findFirst20BySensorIdOrderByDataCampioneDesc(sensorId);
		List<SensorValueDTO> sensorValues = new ArrayList<SensorValueDTO>();
		if(sensors != null && !sensors.isEmpty()) {
			for(SensorValues sensor : sensors) {
				sensorValues.add(SensorValueToDTO.map(sensor));
			}
		}
		else {
